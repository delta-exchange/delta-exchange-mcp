...
`get_connection_status` is registered unconditionally, reconciles safe external file changes, and reports `{environment, credentials_configured, account_tools_available, trading_tools_available, client_name, version, view_build, suggested_next}` — never a key, secret, or fingerprint. `suggested_next` is a short list of things to try next, phrased for the assistant to relay or act on — it names `setup_credentials` and the `get_started`/`market_brief` prompts with no key, and a portfolio check / market brief once the account surface is live. `client_name` is self-reported and carries no authority. `view_build` is `form.build_id()`, a 10-character SHA-256 prefix of the exact `VIEW_HTML` bytes the process would serve. It exists because `version` is identical on every commit of a branch and so cannot distinguish a client that fetched from one that reused a cached build — a question that cost several round trips of reading package caches, and once cost them against the wrong machine entirely. Ask the assistant for the connection status and compare `view_build` against `uv run python -c "from delta_exchange_mcp import form; print(form.build_id())"` before drawing any conclusion from how a rendered form looks. It exists because the save tools are hidden from the model, so after a save the model cannot see whether it worked; without this it has no way to answer "am I connected?".
...
`prompts.py::register(mcp)` is called unconditionally from `build_server`, right after `market.register`. It adds three onboarding prompts (`get_started`, `market_brief`, `portfolio_check`) — the only channel that can put words in front of the *user* rather than the model, since MCP has no on-connect greeting of its own. All three are registered once and never removed, including `portfolio_check`, which has nothing to check without an account: `PromptManager` has no public `remove_prompt` to mirror the tool arm/disarm pattern above, so its own text tells the assistant to call `setup_credentials` first when the account tools aren't up, rather than the server hiding the prompt.
...
2. **Retry policy** — 429 backs off using the `X-RATE-LIMIT-RESET` header (ms); 5xx uses exponential backoff; transport errors retry too. All three retry paths are GET-only — POST/PUT/DELETE never auto-retry, because a mutation whose response was lost may already be applied and a resend can place a duplicate order. A mutation transport failure raises `DeltaApiError` with `execution_outcome_unknown` (sent, response lost — reconcile before resubmitting) or `upstream_unreachable` (connect failure — nothing was sent). `mutation_tool` replaces the unknown-outcome context with the reads that settle that tool's state, from `_OUTCOME_UNKNOWN_BY_TOOL`. Regression tests: `test_a_mutation_is_never_resent_after_a_transport_failure`, `test_a_mutation_connect_failure_says_nothing_was_sent`, `test_unknown_outcome_names_the_correct_state_checks`, `test_get_transport_errors_still_retry`.
...
7. **Hot rebind is one atomic state swap** — `_request` captures `_ClientState` before its first await, and `rebind()` replaces the config, signing prefix, and transport together. A retired transport closes when its final active request or pinned operation exits, with server shutdown as the backstop. Every dispatched trading tool also enters `client.pin()`, so a credential rebind landing mid-call cannot sign part of that call with a different key.
...
- `account_identity.py` owns the API-key account identity: `fetch_account_identity(client)` validates `result.user_id` from `/users/trading_preferences`. Credential checks, the account tool, and close-all resolution share it so the endpoint and response contract cannot drift.
- `credentials.py` is the shared domain: `check(env, key, secret)` verifies the key through the identity helper, and `save(env, key, secret)` writes the key, secret and environment together. Neither front-end owns these. `Check.code` carries Delta's own error code beside the rendered message so a caller can branch on which failure it was without matching on that message's text.
...
- `login.py` is the terminal front-end. It refuses a non-TTY stdin on purpose — `getpass` reads a pipe rather than rejecting it, so `echo $KEY | ... login` would put the secret in shell history. It reads the key and secret itself in cbreak mode (`msvcrt` on Windows), echoing one `*` per character. Prompt and stars go to `/dev/tty` (`CONOUT$` on Windows), not stdout. A lone escape byte (nothing behind it within `_ESC_WAIT`) is dropped; whole sequences (arrow keys, bracketed-paste markers) are skipped.
...
**Spacing lives on a `.field` wrapper, one per question, not on the controls.** Putting it on the controls cannot express a note that is empty most of the time: a bottom margin leaves a hole when there is no note and doubles the gap when there is one.
...
- `close_all_positions` needs `user_id`; it is resolved from `/users/trading_preferences` through `account_identity` and cached per base URL and key in the `register` closure — never a tool param.
...
There is no mode variable. `tools/trading.py` registers whenever credentials are present.
