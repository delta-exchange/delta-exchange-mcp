...
- `login.py` is the terminal front-end. It refuses a non-TTY stdin on purpose — `getpass` reads a pipe rather than rejecting it, so `echo $KEY | ... login` would put the secret in shell history. It reads the key and secret itself in cbreak mode (`msvcrt` on Windows), echoing one `*` per character.
