...
- `login.py` is the terminal front-end. It refuses a non-TTY stdin on purpose — `getpass` reads a pipe rather than rejecting it, so `echo $KEY | ... login` would put the secret in shell history. It reads the key and secret itself in cbreak mode (`msvcrt` on Windows), echoing one `*` per character. Like `getpass` it talks to `/dev/tty` (`CONOUT$` on Windows), not stdout, so a redirected stdout cannot hide the prompt. An escape byte with nothing behind it within `_ESC_WAIT` is the Escape key and is dropped; a whole sequence (arrow key, bracketed-paste marker) is skipped.
