...
machine. Paste it and leave it alone — it fetches the steps and takes the right route, and
it never asks you what appeared on screen.

```text
Install the Delta Exchange MCP server for me. Fetch this file and follow it exactly:
https://raw.githubusercontent.com/delta-exchange/delta-exchange-mcp/main/AGENT-INSTALL.md

Do not ask me to choose, and do not ask me what I can see on screen. If you cannot fetch
a URL, tell me that and hand me the link. Do not improvise an install from memory, and do
not write a config block for me to paste.
```

The steps are in [AGENT-INSTALL.md](AGENT-INSTALL.md). An agent fetches them as it runs, so
it follows the current ones rather than whatever was pasted months ago, and there is one
copy of them to keep right. [The docs site](https://mcp.delta.exchange/docs) hands out the
same prompt and points at the same file.

Why those steps are shaped like that:
