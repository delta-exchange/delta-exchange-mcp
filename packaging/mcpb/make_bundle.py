...
LONG_DESCRIPTION = (
    ...
    "and you get market data only — the two are always used together. The key must have "
    "permission for every account or trading endpoint that the server calls.\n\n"
    "**Market data needs no setup** — leave the API key and secret empty and everything "
    "except your own account still works.\n\n"
    "**To see your account**, create a key at delta.exchange under Account → API Keys. "
    "Current Delta documentation does not establish whether **Read Data** alone is "
    "sufficient. Authenticated testnet verification of the exact Read Data and Trading "
    "permission matrix is still pending. Both credential parts are shown only once, at "
    "creation. Paste them into Configure. Your key is stored by this app and is sent only "
    "to Delta's API, from your own machine.\n\n"
